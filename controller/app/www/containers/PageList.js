...
export const PageSubLink = ({to, spinner, icon, checkbox, selected, onChange, children, style, ...props}) =>
    <NavLink to={to}
        ...
        <Row className="fill">
            ...
            { (spinner||icon) && <Circle radius={40} className="align-center" style={ styles.sublink.circle }>{ spinner ? <Spinner/> : <Icon src={icon+`?${Date.now()}`} style={ styles.sublink.icon }/> }</Circle> }
            {children}
        </Row>
    </NavLink>
