...
export const PageLink = ({to, title, icon, disabled, onClick, children, style, ...props}) =>
    <NavLink to={to} activeClassName="active"
        ...
        {...props}>
        <Col>
            { icon && <Icon src={icon} style={ styles.link.icon }/> }
            <span>{title}</span>
            {children}
        </Col>
    </NavLink>
...
export const PageSubLink = ({to, spinner, icon, checkbox, selected, onChange, children, style, ...props}) =>
    <NavLink to={to}
        ...
        <Row className="fill">
            ...
            { (spinner||icon) && <Circle radius={40} className="align-center" style={ styles.sublink.circle }>{ spinner ? <Spinner/> : <Icon src={icon+`?${Math.floor(Date.now()/2000)}`} style={ styles.sublink.icon }/> }</Circle> }
            {children}
        </Row>
    </NavLink>
